...
- 最后更新：2026-06-14
- 当前 HEAD = c2e620a（T-008a 用户改密码 + status 假能力修复）。提交链 f42dc80 → 4e16f91（T-005b-4 列表查询能力 + 操作人可读化）→ c2e620a（T-008a）。两片独立双推、三方 HEAD 一致。
...
### 下一步（计划）
**T-005b-4 + T-008a 双推归档（HEAD c2e620a）。当前格局：T-005b 后端债剩 T-005b-3（加密参数）+ 文案低优批；中途因 daxing 验收暴露插入「用户/角色管理补缺片」（T-008 系列），PM 裁定补缺片插队到 T-005b-3 之前先做（让 demo 权限体系在界面上可装配，否则后续任何涉非超管角色的验收都撞墙）。**
...
- **T-008b（下一片·用户分角色，前端 + 后端小补回填）**：前端用户页加「分配角色」多选弹窗（listRoles 取选项 + 回填已授 + PUT /sys/users/:id/roles 全量覆写，perm sys:user:assign，入参 role_ids hashid 数组）。**后端小补搭车**：GET /sys/users/:id 现只预载 Posts 不返 roles（SysUser model 无 Roles 字段，user_service.go:146）→ 补「查用户当前已授角色」作回填来源（预载 roles 或单独端点，hashid 出参）。轻-中。
- **T-008c（压轴·角色授权树，前端最重 + 后端小补回填）**：前端角色页加 el-tree 勾选授权（消费 GET /sys/menus/tree + 回填已授 menu_ids + 半选/check-strictly 处理 + PUT /sys/roles/:id/menus 全量覆写，perm sys:role:assign，入参 menu_ids hashid 数组）。**后端小补搭车**：GET /sys/roles/:id 现不返已授 menu_ids（SysRole model 无 Menus 字段，role_service.go）→ 补「查角色当前已授菜单」作回填来源（**因 AssignMenus 全量覆写，回填必须拿到当前全量 menu_ids 否则覆写丢权限→回填是正确性前提非锦上添花**）。复杂度集中在树半选 + 全量覆写不丢权限，验收重点验「回填全量→改一处→提交不误删其他权限」。出 C 任务书前 PM 让执行端专门核实半选 check-strictly vs 级联 + 覆写回填原子性。
...
- **T-005b-3 配置中心加密参数写链路**：CreateConfigInput 无 is_encrypted、Update 全量明文覆写 → 经 API 无法建加密参数、改加密行破坏密文。补"加密参数更新语义"（碰 GCM 信封 + 配套明文查看授权流程 sys:secret:view 落地）。体量最大，单独切。落地后前端放开加密行编辑。
...
- **剩余顺序（PM 现序）**：**T-008b（用户分角色）→ T-008c（角色授权树，压轴）→ T-005b-3（加密参数，体量大）→ 文案批收尾**（补缺片插在 T-005b-3 前，理由见上）。
...
- **待办（角色管理页缺"分配菜单权限"界面·T-007b 遗留，T-005b-1 验收时 daxing 发现）→ 已排期 T-008c（压轴）**：角色管理页只有角色 CRUD，**没有"给角色分配菜单/权限"的界面**。后端 AssignMenus 接口已有（PUT /sys/roles/:id/menus，sys:role:assign，全量覆写）但前端未做 → 当前各角色权限**仅靠 seed 写入**，界面无法查改。**已纳入「用户/角色管理补缺片」T-008c（角色授权树，压轴）**，需后端搭车补「查角色已授 menu_ids」回填来源（全量覆写下回填是正确性前提）。详见上方「下一步（计划）」补缺片段。
