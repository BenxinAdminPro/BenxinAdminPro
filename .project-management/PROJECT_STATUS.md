...
- 最后更新：2026-06-14
- 当前 HEAD = 1d4b9aa（T-008b 用户分角色 + 列表角色列）。提交链 c2e620a（T-008a）→ 75c2b7a（账本）→ 5e83b43（docs 归档）→ 1d4b9aa（T-008b feature）。独立双推、三方 HEAD 一致。
...
### 下一步（计划）
**T-008b 双推归档（HEAD 1d4b9aa）。当前格局：用户/角色管理补缺片做到第二片（T-008a 改密码/status + T-008b 用户分角色/列表角色列），下一片 = T-008c 角色授权树（本批最重·压轴，做完权限体系闭环）。T-005b 后端债剩 T-005b-3（加密参数）+ 文案低优批，排在补缺片之后。**
...
- **T-008c（下一片·压轴·角色授权树，前端最重 + 后端小补回填）**：前端角色页加 el-tree 勾选授权（消费 GET /sys/menus/tree + 回填已授 menu_ids + 半选/check-strictly 处理 + PUT /sys/roles/:id/menus 全量覆写，perm sys:role:assign，入参 menu_ids hashid 数组）。**后端小补搭车**：GET /sys/roles/:id 现不返已授 menu_ids（SysRole model 无 Menus 字段，role_service.go）→ 补「查角色当前已授菜单」作回填来源（同 T-008b A 口径预载，**因 AssignMenus 全量覆写，回填必须拿到当前全量 menu_ids 否则覆写丢权限→回填是正确性前提非锦上添花**）。复杂度集中在树半选 + 全量覆写不丢权限，验收重点验「回填全量→改一处→提交不误删其他权限」+ **闭环验「给 editor 角色授几个 M/C 菜单 → editor/temp05 登录即时可见」（兑现 T-008b 留下的 editor 补验）**。出 C 任务书前 PM 让执行端专门核实半选 check-strictly vs 级联 + 覆写回填原子性。**做完权限体系闭环：用户挂角色（T-008b）+ 角色挂菜单（T-008c）两段界面均可装配。**
...
- **剩余顺序（PM 现序）**：**T-008c（角色授权树，压轴·下一片）→ T-005b-3（加密参数，体量大）→ 文案批收尾**（补缺片插在 T-005b-3 前，理由见上）。
...
- **待办（角色管理页缺"分配菜单权限"界面·T-007b 遗留）→ 已排期 T-008c（压轴·下一片）**：角色管理页只有角色 CRUD，**没有"给角色分配菜单/权限"的界面**。后端 AssignMenus 接口已有（PUT /sys/roles/:id/menus，sys:role:assign，全量覆写）但前端未做 → 当前各角色权限**仅靠 seed 写入**，界面无法查改（**T-008b 验收时坐实：editor 角色 seed 只授 1 个 F 码无 M/C 可见菜单 → editor 登录空菜单，正是此缺口的真实后果**）。**T-008c（下一片）= 角色授权树**：后端搭车补「查角色已授 menu_ids」回填来源（全量覆写下回填是正确性前提），前端 el-tree 勾选 + 半选/check-strictly + 全量覆写。**含 editor 补验**：T-008c 给 editor 角色授几个 M/C 菜单 → editor/temp05 登录即时可见，兑现 T-008b 留下的 editor 闭环补验。详见上方「下一步（计划）」补缺片段。
