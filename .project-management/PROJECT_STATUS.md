...
- 最后更新：2026-06-15
- 当前 HEAD = {待双推核验·报告归档hash}（T-009b 文案/低优债批收尾 · 报告归档提交 = 最终 HEAD）。提交链 {feature hash}（T-009b feature）→ {账本 hash}（账本）→ {报告归档 hash}（报告归档 = 最终 HEAD）。本片双推后三方 HEAD 一致（待执行端双推后 ls-remote 核验，回报后 PM 补字面对账）。
  - 上一收官：HEAD 6503cfe（T-009a 后台禁搜索引擎收录）。提交链 a4b3fc8→01e83a0→6503cfe。
...
| T-009b | 文案/低优债批收尾（file 6+config 2=8 键文案 + 菜单父节点专属 ErrInvalidParentMenu + SetStatus 同值幂等不误返 404） | ✅ 至 {待填}（feature {待填}）。8 键补 defaultMessages 消除返裸 key；新码 offset 47 段尾纯追加快照仍 41；SetStatus 三态 SQLite+MySQL 双证（含「不存在仍 404」负例）。openapi v0.14.1。详见上方记录。 |
...
- **待办（T-003d-fix 单列）**：TestNewEnforcerMySQL_RoleInheritance 在 78342cf 即红（与 T-003e/T-007d 无关），是 T-003b 改 model.conf 为 perm code 精确匹配时漏更新的陈旧 URL keyMatch 断言，非真鉴权 bug。**处理顺序铁律：先确认角色继承经 perm code 路径有覆盖，再重写/删除该旧断言，不许直接删了凑绿。** 附 CI 卫生缺口：rbac `-tags=integration` 在阶段一收官时这一标签未被真跑过。**🔺升级提醒（T-009b 记）：此红自 78342cf 阶段一收官即挂、历经多片仍未处理（T-005b-3/T-009b 均 stash 干净 HEAD 实证同样 FAIL、非各片引入），不应长期悬空——建议尽快单独立项按上述铁律处理，避免「长期红」麻痹 CI 卫生。**
