...
- 最后更新：2026-06-16
- 当前 HEAD = 5a79b47（T-011a x-table 多选基建 · feature 双推收官 = 当前真实远程 HEAD）。账本 + 报告归档 commit 待执行端落地后 HEAD 前移；提交链 5a79b47（T-011a feature）→ {{LEDGER_HASH}}（账本）→ {{FINAL_HEAD}}（报告归档 = 最终 HEAD）。feature 双推后三方 HEAD 一致（LOCAL==Gitee origin==GitHub github，ls-remote 实测 5a79b47；精确 stage 仅 3 文件、账本 M 与 report 未混入；Gitee 本次未撞 fake-IP 直推）。
  - 上一收官：HEAD 131e870（T-010b CI 接入 GitHub Actions）。提交链 dfc5fc1（feature）→ f5e2ded（账本）→ 131e870（报告归档 = 最终 HEAD）。
  - 再上一收官：HEAD f6d5ca4（T-010a 集成测试连接 env 收口）。提交链 e202d92→ccfeb82→f6d5ca4。
  - 再上一收官：HEAD 16bf4d3（T-003d-fix casbin pre-existing red 清零）。提交链 ca7e8db→23e2e4e→16bf4d3。
  - 再上一收官：HEAD dacc8a1（T-009b 文案/低优债批收尾）。提交链 c9db3a9→…→dacc8a1。
  - 再上一收官：HEAD 6503cfe（T-009a 后台禁搜索引擎收录）。提交链 a4b3fc8→01e83a0→6503cfe。
  - 再上一收官：HEAD d89e3e2（T-005b-3 配置加密参数写链路）。提交链 1d21cf4→2b96a09→d89e3e2。
...
- 🏁 **T-009a 后台禁搜索引擎收录双推归档（HEAD 6503cfe，feature a4b3fc8）= 新需求池首条落地，noindex 三件套 + 参数化权威控制点。** 让管理后台默认禁止被搜索引擎收录（登录页/接口路径被爬是攻击面），且禁与否由部署配置控制（守底座中立，私有化可关）。9 文件 +285/-6（后端新建 httpmw 包 + 中间件 + 单测 + 集成测试 + demo 装配/config + config 库参考结构体；前端 robots.txt 新增 + index.html meta）。**三件套**：① **后端 X-Robots-Tag 全局中间件（唯一碰 Go、参数化权威控制点）**——新建 `httpmw` 通用 HTTP 中间件包 + `XRobotsTag(RobotsConfig)`，挂 engine 级 `r.Use`（Recovery 后、所有路由前）对**所有响应无差别注入**（含鉴权失败 401/未匹配 404，全栈测试坐实）；开关 + 内容走静态配置，`DefaultXRobotsTag="noindex, nofollow"` 常量单一定义、逻辑不硬编码字面。② **admin `public/robots.txt`**（`User-agent: * / Disallow: /`，Vite 拷站点根）。③ **admin `index.html` meta** `<meta name="robots" content="noindex,nofollow">` 构建期兜底。**默认安全（命门）**：loadConfig `v.SetDefault("security.x_robots_tag.enabled", true)` + content 默认 → 配置整段省略也走禁收录（viper GetBool 缺省返 false=不安全，SetDefault 兜底）+ 中间件内容空回退 DefaultXRobotsTag（开了不注入空头）。**前端 A1 裁决（daxing 定）**：robots.txt/meta 构建期写死，**不做运行时/构建期 env 联动**（静态产物兜底件不值得引构建变量；权威可配置点是后端中间件）；私有化要放开 = 关后端开关 + 手删 robots.txt/meta（部署文档说明）。**底座中立**：开关/内容全配置注入；新建 httpmw 包不绑应用、为将来其他安全响应头（CSP/X-Frame-Options 等）预留，本片只放 X-Robots-Tag。库 canonical `config.AppConfig` 加 SecuritySection/XRobotsTagSection 保参考结构完整（附「消费方须 SetDefault 保默认安全」责任注释）。**测试**：中间件四态单测（开默认/关/自定义/开但内容空回退）+ 常量值锁定（httpmw/robots_test.go，进默认闸门）；全栈 e2e（robots_integration_test.go 真 buildApp）坐实 401/404 响应均带头 + 内容回退 + 自定义 + toggle 关无头。**daxing 验收**：curl 响应头含 X-Robots-Tag（含 404 全局注入）、/robots.txt 返 Disallow: /、页面 head 含 meta——功能全过。**退回修复（编码缺陷）**：验收发现 robots.txt 套用源码五项中文代码头注释致乱码 → 选项 B 去中文头改纯 ASCII（一行英文注释 + 协议两行），`file` 报 ASCII text、无 BOM、dist 产物正确；index.html 核查 UTF-8 无 BOM、meta 行纯 ASCII（未动）。**规范澄清（PM 记账本）：robots.txt 是机器读的标准协议文件、非源码，不纳入「五项代码头注释」规范。** 无 DDL/错误码/权限码/openapi 端点。**新需求池禁收录条目至此清零；媒体管理条目仍待先摸底再拆。**

- 🏁 **T-009b 文案/低优债批收尾双推归档（HEAD dacc8a1，feature c9db3a9）= 三项内聚低优后端债清零（零行为外扩）。** ① **渲染层补 8 键文案**（file 6 + config 2，PM 裁定 A 搭车）——码早注册、补 `defaultMessages` 消除返裸 i18n key：file 6 键（修 T-007f §8-2）+ config 2 键 `sys.config_decrypt_failed`(11080)「参数解密失败」/`sys.migration_failed`(11081)「数据库迁移失败」（§8 范围外发现搭车补，同文件同类）。② **菜单父节点专属码 `ErrInvalidParentMenu`**（offset 47，RBAC 段 40~49 段尾纯追加、既有码值零变更、无冲突、48/49 仍留空），menu_service.go 三处由复用 `ErrInvalidParentDept` 改用新码 + 文案「无效的父菜单」（修 T-007g §8-2）。③ **SetStatus 同值 404 修复**（修 T-007h 观察）：`RowsAffected==0` 改 Count 探测存在性，区分「无变更幂等成功」vs「记录不存在仍 404」——**MySQL 同值改动行返 0 误判 404、SQLite 返 1 不暴露**，命门由 `-tags=integration` MySQL 坐实（吃「单测绿≠装配能跑」教训）。**SetStatus 三态双证**（SQLite 单测 + MySQL 集成）：同值→成功不 404 / **不存在→ErrUserNotFound（存在性未放宽，关键负例）** / 真变更→落值；Count 走 Model(&SysUser{}) 带软删 scope 不误判软删用户为存在。**回归快照仍 41 无误加码**（8 键纯补文案、config 2 码本在 41 内、菜单新码 47 是 RBAC 段内既留位的下一空码——核实 len(specs)==41 通过坐实未新增段）。openapi 错误码枚举 +1（v0.14.0→v0.14.1，无端点/schema 变更）。8 源/测试文件 +323/-4（errcode.go/render.go/menu_service.go/user_service.go + migration_test/render_message_test/service_test/org_integration_test）。**留账本待办（PM 裁定，不在本片碰）**：getByID 第 4 处 ErrInvalidParentDept（编辑不存在菜单返「父部门」）轻微语义瑕疵单独立项；casbin TestNewEnforcerMySQL_RoleInheritance pre-existing red（干净 HEAD stash 实证同样 FAIL、自 78342cf 即红）单独立项。**执行端规范表现**：放行指令引用「PM 草稿」但草稿未附时,执行端拒绝凭空编造账本提交、停下回报（守 T-006 改 PROJECT_STATUS 权限属 PM），账本由 PM 产出。commit c9db3a9。

- 🏁 **T-003d-fix 双推归档（HEAD 16bf4d3，feature ca7e8db）= casbin pre-existing red 清零，自 78342cf 悬空的测试债收口。** 性质=修一条红测试 + 正向证明角色继承真生效，非加功能。摸底四口径坐实岔路 A（stale URL 断言非鉴权 bug）：matcher 现行 `r.obj==p.obj` 精确相等，旧测试仍用 keyMatch2 时代 `/api/admin/*` URL 通配，精确不等→拒。**处理顺序铁律守住**：先经口径 3 确认继承在 perm code 模型有充分其他覆盖（TestEnforcerRoleInheritance 同 model.conf 只换 file adapter、默认闸门常绿；TestPolicySyncUserRoles 用户→角色→perm 传递；中间件全链路；demo），**再**动旧断言。**PM 裁定重写不删**（区别于裸删凑绿）：该测试是唯一在真实 MySQL+gorm-adapter 持久化路径跑 g 规则继承的用例，删则丢此层覆盖。重写=obj/act URL→perm code（sys:user:list/access，镜像 rbac_test.go:83 元组）+ **同主体负向对照**（alice 无 g 链 deny→加链 allow，且 alice 无直挂 p 规则故 allow 唯一归因继承链——守「正向 enforcement 证据而非仅缺席失败」/「enforce 即时翻转」范式）+ **act 通配保留**（另加 admin 持 (sys:user:create,*)、alice 经继承 anyact 命中，原 act=* 覆盖未静默丢）+ bob 无角色 deny 基线。**防假绿自检（T-007i 范式）打在最承重的负向对照断言**：改坏「无链应 deny」→真 FAIL（line 149）→还原 PASS，证测试非空跑真在断言。rbac `-tags=integration` 全套 94 PASS/0 FAIL、`-count=1` 可重复、无连带红/隐藏红；默认闸门绿；build/vet 净。1 测试文件改动（mysql_integration_test.go），未动 model.conf/业务逻辑/openapi/DDL/码。**CI 卫生根因上报：全仓无任何 CI 流水线配置，integration 标签从未自动跑=这条红能从 78342cf 长期悬空的根因，已记待办。** commit ca7e8db。

- 🏁 **T-010a 双推归档（HEAD f6d5ca4，feature e202d92）= 质量闸门基建批次（T-010）首片，集成测试连接参数 env 收口、CI 接入软前置铺平。** 把全仓集成测试 9 处硬编码 localhost MySQL/Redis 连接统一为「读 `BENXIN_TEST_MYSQL_DSN`/`BENXIN_TEST_REDIS_ADDR`、默认 localhost」，抽 `internal/testsupport`（`//go:build integration`）单点维护默认值/env 名，退役重复命名 `DEMO_E2E_*`，命名空间统一 `BENXIN_TEST_*`（与既有 migrator 样板天然一致）。**T-010 摸底 settle 掉两个预想前置**：A1 全仓 integration 266 PASS/0 FAIL（rbac 外无存量红）→ CI 接上即绿、无需前置清红片；A3 全仓仅 1 处合理 t.Skip（captcha 导样图）、集成测试本就 `//go:build integration` + 连不上 `t.Fatalf` fail-loud → 无假绿温床、无需 fail-loud 改造片；C3 test-only CI 自带固定测试密钥、不读业务密钥 → CI 无需注入业务 secrets。**零回归硬约束**：env 缺省默认值与原硬编码逐字一致，不设 env 本地 integration 原样 266 PASS。**env 真生效正向证据（命门·防「改了没读 env」假绿）**：MySQL/Redis 双向坐实——`export BENXIN_TEST_*=错端口`(3399/6399)→真 FAIL(dial 到错端口证 env 被消费)→unset→PASS。**helper 选型**：共享 helper（无 import cycle、仅 import os）；验掉 `//go:build integration` 包在 `go build ./...`(无 tag) 下被静默跳过不破构建（/tmp+真仓 exit 0 实证）。**偏差①（PM 认领·任务书枚举疏漏）**：§2 MySQL 清单漏列 rbac/org_integration_test.go（但摸底 B1 已标 + §1 全仓统一 + §7 grep 无残留要求覆盖），执行端按目标/验收补改 org，PM 裁定保留本片（漏则 §7 grep 闸门反而过不了）。**偏差②（搭车清理零风险）**：退役 DEMO_E2E_* 连带删失效的 defaultE2E* const/envOr func/os import。**127.0.0.1→localhost 归一**：demo e2e 默认由 127.0.0.1 改 localhost（功能等价），但 FAIL 输出现 `dial tcp [::1]` 揭示 localhost 本机先解析 IPv6 ::1 → **T-010b watch-point：CI runner 若 service 只绑 IPv4，应显式注入 BENXIN_TEST_MYSQL_DSN 用 127.0.0.1 而非依赖 localhost 默认**（已记）。11 文件 +98/-39（1 新增 testsupport/conn.go + 10 改集成测试），未升 openapi、无 DDL/错误码/权限码、未碰断言/依赖门控形态/app 运行时配置层。commit e202d92。

- 🏁🏁 **T-010b 双推归档（HEAD 131e870，feature dfc5fc1）= 质量闸门基建批次（T-010）收官片，CI 根因债正式闭合——底座首次拥有自动测试闸门。** 新建 `.github/workflows/ci.yml`（`on: push`(main) + `pull_request`(main)，`runs-on: ubuntu-latest`，`permissions: contents: read`）：service container mysql:8.0(库 benxinadminpro/root/root/healthcheck mysqladmin ping) + valkey/valkey:8(healthcheck valkey-cli ping)，steps = checkout@v4 → setup-go@v5(`go-version-file: server/go.mod` 钉 1.25.0 不臆造) → build → vet×2(默认+integration tag) → Wait MySQL ready(`mysqladmin ping -h 127.0.0.1`) → **`go test ./...`(默认闸门)** + **`go test -tags=integration ./...`(integration 闸门)**；integration 步注入 `BENXIN_TEST_MYSQL_DSN`(=testsupport.DefaultMySQLDSN 逐字、仅 host→**127.0.0.1** 规避 ::1)/`BENXIN_TEST_REDIS_ADDR=127.0.0.1:6379`（T-010a watch-point 闭合）。+README CI badge（原空文件）。**根因实锤**：首跑前 Actions `total_count=0` 坐实"全仓史上从无 CI、integration 从未自动跑"=T-003d-fix 那条红能从 78342cf 烂 9 片没人拦的根因。**双证（防假绿命门·两条触发线都验）**：① 首跑绿(run#1 push 触发 dfc5fc1)——10 步全绿、默认+integration 双 gate 真跑(非 skip)、127.0.0.1 真连 service container 无 ::1 抖动；② **假红真拦**(PR#1 pull_request 触发)——run#2 改坏 TestDefaultXRobotsTagValue 断言→`failure`(默认门红即拦下，integration 步 fail-fast 跳过) → run#3 改回→`success`(双 gate 均绿)，证闸门对真红绝不放行、撤回即恢复。**假红验证零污染**：临时分支 ci/fake-red-verify(off dfc5fc1)承载破坏 commit 8a885e2→改回 288b07d，经 PR#1 验证(开 PR 由 daxing 浏览器、删远程分支由 daxing PR 页操作，守"远程/凭证归 daxing"分工) → PR 关闭(不合并)+分支删除，**破坏 commit 从未进 main、三方仓库发布历史均无**(`git diff main -- robots_test.go`空、`ls-remote --heads github ci/fake-red-verify`空)。**CI 片专项**：test-only CI 不注业务 secrets(集成测试自带固定测试密钥)、第三方 action 钉官方主版本、ci.yml 纯 ASCII(非源码不纳五项注释规范，同 T-009a 先例)。**双推走 CI 片特殊流程**：先仅推 GitHub 触发 Actions 验证→PM 评审证据放行→补推 Gitee(无 fake-IP 劫持)→ls-remote 三方一致 dfc5fc1。**PAT scope 插曲**：首推被拒(PAT 缺 workflow scope)→daxing 加 scope+更新 keychain 重推成功(凭证由 daxing 掌管、执行端未碰 token 明文)。2 文件(ci.yml 新增 + README 改)，未改任何 Go 业务/测试、未升 openapi、无 DDL/错误码/权限码。commit dfc5fc1。**至此 T-003d-fix→T-010a→T-010b 三片把"casbin 红清零→env 收口→CI 上线"串成完整闭环，「红能烂 9 片没人拦」的根因债从此堵死——存量/新增真后端红将被 push/PR 自动闸门当场拦截，不再靠人肉跑。**

- 🏁 **T-011a x-table 多选基建双推归档（feature HEAD 5a79b47）= 媒体管理批次（T-011）首片，给 x-table 补多选承重基建。** 纯前端、零后端 Go / 零 openapi（v0.14.1 不升）/ 零 DDL / 零错误码·权限码。3 文件 +85/-6（types.ts / XTable.vue / dev/xtable-harness/index.vue）。**摸底头号利好兑现**：XTable `<el-table :row-key="rowKey">` 早已绑（XTable.vue:298-305）→ 多选硬前置 row-key 现成、零新增/零改、PM 最初担心的 row-key 缺位风险不存在。**实现 = optional config gated**：`selectable?: boolean`（缺省 false）为真才前插 `type=selection` 列 + `@selection-change`→`selectedRows` ref + `clearSelection()`；`defineExpose` 加 selectedRows/clearSelection（refresh 不变）；新增 `#batch-actions` 作用域插槽（slot props {selected, clear}）仅 selectable 时渲染（showBatchActions 并入 showToolbar，off 页该 OR 项恒 false）。同 T-007c readonly / T-007g field-slot「缺省即旧行为」范式。**页内多选语义（v1·裁决②）**：fetchData 落表清 selectedRows + 无 reserve-selection → 翻页/筛选/刷新清选中；跨页 reserve-selection 留待 gated flag（row-key 已具备故将来极廉，记待办）。**裁决③ cell-slot 不并入**（多选与单元格插槽两套正交机制，cell-slot 单独排片，首消费者 = 加密行 🔒）。**jsdom 组件测试豁免**（任务书允许、PM 接受）：零回归是结构性 gating 非分支逻辑、selection-change DOM 驱动 jsdom 下脆、为单片铺组件测试基建违 T-007i scope → harness（on/off 对照）+ typecheck/build + daxing 逐页视觉兜底，devDeps 零新增。pnpm build exit 0 / typecheck 净 / pnpm test 既有 17 PASS（tree.spec 未动）。**daxing 验收三过**：① harness 多选卡闭环（勾选→已选 N 联动→批量 toast 列名 item-01/02/04→清空归零→翻页/刷新清空）② off 对照卡无选择列无位移 ③ **8 消费页逐页扫**（user/role/dict/config/operlog/loginlog/file/post）无多选列、布局无位移、工具栏/筛选/排序/操作列原位。**纪律记账（CRUD 闭环验收有意识降级·非遗漏）**：第③项「抽 2~3 页真点 CRUD 闭环」未做，PM 评估本片纯前端 UI 结构性加法、全 gated on selectable、逻辑极薄、8 页列表正常渲染已强佐证零回归、无后端类潜伏装配坑 → 有意识接受降级（区别于 T-007b/c「验收没点进去」的遗漏）；**沉淀：UI 结构性加法（缺省分支不变即零回归）的 CRUD 闭环验收可按 PM 判断降级；写路径/装配/ORM scope 类切片不可降级。** commit 5a79b47。**T-011a 收官，多选承重基建立住，T-011c 媒体批量删可消费。**
...
| T-009a | 后台禁搜索引擎收录（noindex 三件套：X-Robots-Tag 中间件参数化 + robots.txt + index.html meta） | ✅ 至 6503cfe（feature a4b3fc8）。**新需求池首条**。新建 httpmw 包全局中间件无差别注入（含 401/404）；默认安全 SetDefault 兜底；前端兜底件构建期固定（A1 裁决）。中间件四态单测 + 全栈 e2e 坐实。退回修复 robots.txt 编码（去中文头改纯 ASCII）。无 openapi/DDL/错误码。详见下方记录。 |
| T-009b | 文案/低优债批收尾（file 6+config 2=8 键文案 + 菜单父节点专属 ErrInvalidParentMenu + SetStatus 同值幂等不误返 404） | ✅ 至 dacc8a1（feature c9db3a9）。8 键补 defaultMessages 消除返裸 key；新码 offset 47 段尾纯追加快照仍 41；SetStatus 三态 SQLite+MySQL 双证（含「不存在仍 404」负例）。openapi v0.14.1。详见上方记录。 |
| T-003d-fix | 清 casbin TestNewEnforcerMySQL_RoleInheritance pre-existing red（重写 URL 断言→perm code + 同主体负向对照正向证明 g 继承 + act 通配保留） | ✅ 至 16bf4d3（feature ca7e8db）。**测试债清理片**，非加功能。摸底坐实岔路 A（stale URL 断言非鉴权 bug）。重写不删（保唯一 MySQL/gorm-adapter 持久化路径继承覆盖）；同主体翻转（无链 deny→加链 allow，allow 唯一归因继承链）+ act 通配保留 + 防假绿自检打在负向对照断言。rbac -tags=integration 94 PASS/0 FAIL 可重复。CI 流水线缺位（根因）已上报待办。详见下方记录。 |
| T-010a | 集成测试连接参数 env 收口（localhost 硬编码 → BENXIN_TEST_* 可覆盖默认 localhost + 抽 internal/testsupport 共享 helper + 退役 DEMO_E2E_*） | ✅ 至 f6d5ca4（feature e202d92）。**质量闸门基建批次（T-010）首片·CI 接入软前置**。9 处硬编码 MySQL/Redis 连接统一 BENXIN_TEST_MYSQL_DSN/BENXIN_TEST_REDIS_ADDR；零回归（integration 266 PASS 与 T-010 A1 基线逐字一致）；env 真生效双向正向证据（错端口→FAIL→unset→PASS）。闭合 localhost 硬编码待办。11 文件 +98/-39。详见下方记录。 |
| T-010b | CI 接入 GitHub Actions（默认闸门 + integration 双跑 · service container mysql:8.0+valkey:8 · 127.0.0.1 注入 · 假红验证闸门真拦） | ✅ 至 131e870（feature dfc5fc1）。**质量闸门基建批次（T-010）收官片·CI 根因债闭合**。新建 .github/workflows/ci.yml（push+PR 触发，build+vet×2+默认闸门+integration 闸门）+ README CI badge。首跑前 total_count=0 实锤"史上从无 CI"；首跑绿（run#1 push）+ 假红真拦（PR#1 run#2❌→run#3✅，pull_request 路径）双证。2 文件，无 Go 业务改动/openapi/DDL/码/secrets。详见下方记录。 |
| T-011a | x-table 多选基建（selectable gated 选择列 + 选中态暴露 selectedRows/clearSelection + #batch-actions 批量槽） | ✅ feature 至 5a79b47。**媒体管理批次（T-011）首片·纯前端基建**。row-key 复用（早已绑）；页内多选语义（裁决②）；cell-slot/跨页保留留待（裁决③）。缺省零回归（8 消费页扫验 + harness on/off 对照）。jsdom 豁免（结构性 gating）。详见上方记录。 |
...
- ~~【菜单·8-2·文案】菜单父节点错误复用 ErrInvalidParentDept~~（✅ **T-009b 已修**：新增菜单专属码 ErrInvalidParentMenu offset 47「无效的父菜单」，menu_service.go 三处改用新码）：移动成环/父不存在原报「无效的父部门」（11035），语境是菜单却复用部门错误。
...
### 下一步（计划）
**T-011a 双推收官（feature HEAD 5a79b47）。当前格局：媒体管理批次（T-011）启动，先摸底再拆为 3 片——✅ T-011a x-table 多选基建（5a79b47）→ ⏳ T-011b 后端能力片（mime 大类筛 + 批量软删端点，裁决①④已锁、实现书已出，openapi→v0.15.0）→ ⏳ T-011c 媒体消费片（消费多选批量删 + mime tab + 图片预览 + 视频/音频超阈值降级 + 扩 allowed_exts）。a/b 无依赖可并行，c 收口。此前已收官：用户/角色补缺片（T-008a/b/c，权限体系两段界面闭环）+ 后端债（T-005b-1+2+4+3）+ 禁收录（T-009a）+ 文案低优批（T-009b）+ 质量闸门批次（T-003d-fix/T-010a/T-010b，CI 上线把「红能烂 9 片没人拦」根因债堵死）。x-table 后续可选扩展（cell-slot / reserve-selection）已记待办、按需拆。**
...
- **文案/低优批**（✅ **T-009b 已清四项**：~~file 文案缺 6 键~~ + ~~菜单·8-2 错误复用~~（改专属码）+ ~~config 2 键缺文案~~（搭车）+ ~~SetStatus 同值误返 404~~（Count 探测三态）；**剩余未做**：§8-4 dict_type 改名孤儿化（倾向禁改，属行为变更需独立决策非文案债，单列）/ §8-5 悬空 sys:secret:view（绑明文查看、已三轮论证默认不做、码保持悬空）/ 数据展示文案「已注销」「匿名」集中常量将来数据层多语言再统一收（T-005b-4 观察）/ 可读化按名过滤边缘：删用户后旧日志按名过滤不到仅显已注销（T-005b-4 观察，可接受））。
...
- **🆕 约定（自引用 hash 两态分叉·T-003d-fix 摸出固化）**：账本「提交链 feature→账本→报告归档（=最终 HEAD）」里，**账本 commit hash 与报告归档 commit hash 是自引用/前向引用，数学上无法在自身 commit 内容里填实**（commit 不能写自己的 hash）。故**两态分叉、各有真相，非缺陷**：① **git 提交态**——feature hash 填实，账本/最终 HEAD 两处**保留占位**推送（`git show dacc8a1:PROJECT_STATUS.md` 实证 T-009b 当时即如此）；② **知识库态（Derek 重传那份）**——双推后执行端回报两 commit hash，**PM 在工作树副本填实**，这份才是跨对话权威源。**校验闸只查我们的 hash 占位 token（FEATURE/LEDGER/FINAL_HEAD），勿用裸 `grep '{{'`——`{{TABLE_PREFIX}}`(T-005 迁移器记录) 是合法字面量、永久保留。**
...
- ~~待办：rbac/system 既有集成测试硬编码 localhost:3306，理想应改 env 可覆盖（参考 e2e/migrator 已支持 3307）；将来顺手收。~~（✅ **T-010a 已清零，feature e202d92**：全仓 9 处硬编码 MySQL/Redis 连接统一 BENXIN_TEST_MYSQL_DSN/BENXIN_TEST_REDIS_ADDR 可覆盖、默认 localhost，抽 internal/testsupport 共享 helper 单点维护，命名空间统一。零回归 266 PASS + env 真生效双向坐实。）
- ~~**待办（T-003d-fix 单列）**：TestNewEnforcerMySQL_RoleInheritance 在 78342cf 即红……~~（✅ **T-003d-fix 已清零，feature ca7e8db**：摸底坐实岔路 A——matcher 现行 perm code 精确相等，旧断言仍用 keyMatch2 时代 URL 通配（`/api/admin/users` vs `/api/admin/*` 精确不等→拒），是 stale 断言非鉴权 bug。处理顺序铁律满足：先证继承经 perm code 有覆盖（口径 3：TestEnforcerRoleInheritance 同 model.conf/file adapter 默认闸门常绿 + TestPolicySyncUserRoles + 中间件全链路 + demo），**再**重写。**PM 裁定重写不删**：唯一跑在真实 MySQL+gorm-adapter 持久化路径上的 g 规则继承用例，删则丢「继承经真实 DB 适配器」覆盖。重写=URL→perm code（sys:user:list/access，镜像 rbac_test.go:83）+ **同主体负向对照**（alice 无链 deny→加链 allow，allow 唯一来源继承链）+ act 通配保留（另加 (sys:user:create,*) 经继承 anyact 命中，原 act=* 覆盖未丢）+ bob deny 基线。防假绿自检打在负向对照断言（改坏真 FAIL→还原 PASS）。rbac -tags=integration 全套 94 PASS/0 FAIL 可重复。)
- ~~**🆕 待办（CI 流水线缺位·T-003d-fix 上报·根因·高优·T-010b 实现中）**~~（✅ **T-010b 已闭合，feature dfc5fc1**：GitHub Actions CI 上线，push(main)/PR(main) 自动跑默认闸门 + integration 双跑；首跑前 total_count=0 实锤"史上从无 CI"=根因，首跑绿 + 假红真拦双证。**T-003d-fix→T-010a→T-010b 三片闭环：casbin 红清零→连接 env 收口→CI 上线，「红能烂 9 片没人拦」的根因债从此堵死。**）
- **🆕 待办池（CI 可选增强·T-010b 上报·按需另拆·非紧）**：① **admin 前端 CI 闸门**——T-007i vitest 基建已就绪（`pnpm test` 17 项 + `pnpm build`），可低成本加进 ci.yml 或独立 job（Node setup + pnpm install + test + build），让前端回归也进自动闸门。② **Gitee 侧 CI**——本片只 GitHub Actions(闸门跑镜像仓)；若要主仓也可见闸门，另立 Gitee Go 同等流水线（Go+service 成熟度偏弱，按需评估）。③ **workflow 缓存/并发优化**——actions/cache 缓存 go mod、concurrency 取消过时 run，提速降耗。均非紧、用到再拆，不夹带。
- **🆕 待办（低优·菜单 getByID 语义瑕疵·T-009b 上报·PM 裁定单列）**：menu_service.go 第 4 处 ErrInvalidParentDept（getByID :239 附近）——经 Update 编辑一个**不存在的菜单**时外露「无效的父部门」，语义实为「被编辑的菜单自身不存在」（既非「父」亦非「部门」）。T-009b 严守「三处」边界未动（用 ErrInvalidParentMenu 对「自身不存在」亦不精准）。治本需新增「菜单不存在」码（ErrMenuNotFound 类）或复用现有 NotFound 语义。低优、单独微调片。
...
- **🆕 待办（媒体管理·图片/视频/音频·daxing 提·2026-06-15·体量中大需先摸底再拆）**：上传的图片/视频/音频统一有记录、可分类、可上传、可删除/**批量删除**、可查询。**边界划清（守业务中立铁律）**：✅ 进底座 = 「通用文件能力的特化增强」——底座已有 sys_file 通用文件表 + StorageDriver + 鉴权上传/下载（T-004b）+ 文件管理页（T-007f），sys_file 已有 mime 字段（T-007f 核实）→ 按 mime 大类（image/* 视频 video/* 音频 audio/*）分类查询、批量软删、媒体预览属通用能力不绑业务；⛔ 不进底座 = 具体业务的媒体用法（哪张图属哪个商品/会员/内容）留业务侧（BenxinKP）扩展点。**本质 = sys_file 能力增强片**，拆解轮廓：后端（mime 大类分类查询 + **批量软删端点**，现状单条软删无多选见 T-007f §8；缩略图/图片宽高/视频时长等媒体元信息碰解码库，底座是否引单独评估，倾向先不引留扩展点）+ 前端（媒体管理页网格/缩略图视图、类型筛选 tab、多选+批量删、图片预览/视频播放器）。**关键耦合**：x-table 现无多选语义（账本多处记「未触发多选回 PM」）→ **批量删除是 x-table 多选能力首个真消费者**，会逼出 x-table 加 selection 列基建决策（可与累积四次的「x-table 单元格插槽基建片」一并评估）。排期：体量不小 + 耦合 x-table 多选基建，真排时先只读摸底（sys_file mime 现状 + 多选端点 + x-table 多选改造面）再拆。可能拆成「x-table 多选基建片 + 媒体管理消费片」两片。 **【批次启动·2026-06-16】** 已摸底→拆 3 片：✅ **T-011a x-table 多选基建（5a79b47 收官，optional config gated 缺省零回归，row-key 早已绑、页内多选语义、cell-slot/跨页保留留待）** → ⏳ **T-011b 后端能力片**（mime 大类 token→前缀筛 + POST /sys/files/batch-delete 单条 IN bulk 软删幂等 + 复用 sys:file:delete，openapi→v0.15.0，裁决①④已锁、实现书已出）→ ⏳ **T-011c 媒体消费片**（消费多选批量删 + mime 大类 tab + 图片预览 el-image/blob + 视频/音频内联超阈值降级仅下载 + 扩 demo allowed_exts 加 video/audio）。a/b 无依赖可并行，c 收口。媒体元信息（缩略图/宽高/时长）本批不引解码库、留扩展点。**关键摸底事实**：sys_file mime 存客户端 Content-Type 经 ValidateContentType 交叉校验（组织性非安全边界）；下载无 Range/无 tokenized URL（大视频 blob 整载无 seek→视频内联超阈值降级）；批量删全仓无先例（AssignRoles 数组覆写仅作范式参考）。
- **🆕 待办（x-table 后续可选 gated 扩展·T-011a 上报·非紧·按需拆）**：① **列内自定义单元格控件槽 `#cell-<prop>`**——清账本累积「列内控件」诉求前 3 个（T-008a status switch 行操作绕法 / T-008b 角色列 el-tag formatter 绕法 / 加密行 🔒 已加密替裸星号）。与多选正交、同属 optional-config 缺省走 formatter 零回归。**建议以「加密行 🔒 已加密」UX 待办作首个真消费者**（顺带退役前两处绕法），有正当消费驱动再拆、勿投机搭车。② **reserve-selection 跨页保留选中**——T-011a 仅页内多选；row-key 已绑故跨页极廉，做成又一 gated flag，用到再加。
