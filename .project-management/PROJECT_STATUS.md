...
- 最后更新：2026-06-15
- 当前 HEAD = 1d21cf4（T-005b-3 配置加密参数写链路 · 后端债批次第三片）。提交链 f88d3c7（T-008c feature）→ 0be9890（账本）→ 7f07928（报告归档）→ 1d21cf4（T-005b-3 feature）→ 账本 → 报告归档。本片双推后三方 HEAD 一致（ls-remote 核验）。
...
| T-005b-3 | 配置加密参数写链路（新建可加密 + 编辑指针三态不破坏密文 + 解禁前端编辑）+ seed 加密样例 | ✅ 至 1d21cf4。**后端债批次第三片**。ConfigService 经 SetGCMKey 接 GCM；UpdateConfigInput ConfigValue *string 三态（取 DB 现有 is_encrypted 判据，nil=保持）。解密往返三态 + 全栈 enforce + seed 幂等坐实。openapi v0.14.0。T-007d 加密行编辑禁用至此放开。详见下方记录。 |
...
- **剩余顺序（PM 现序）**：~~T-008c（角色授权树，压轴）~~（✅ f88d3c7）→ ~~T-005b-3（加密参数，体量大）~~（✅ 1d21cf4）→ **文案批收尾 / 新需求池（禁收录·媒体管理）排期**。补缺片三片（T-008a/b/c）+ 后端债 T-005b-1+2+4+3 已全收官。
...
- **🆕 待办（后台禁搜索引擎收录·安全/隐私·daxing 提·2026-06-15）**：管理后台需禁止被搜索引擎收录（登录页/接口路径被爬是攻击面）。**三件套（底座统一做）**：① admin 站点根 robots.txt `Disallow: /`（最低成本、只挡守规矩爬虫）② **后端中间件统一注入响应头 `X-Robots-Tag: noindex, nofollow`**（比 robots.txt 强、挡不读 robots 的爬虫，最适合底座=中间件统一控制）③ admin index.html 加 `<meta name="robots" content="noindex,nofollow">` 兜底。**底座中立性=参数化**：是否注入 noindex / robots 内容由配置控制（私有化部署可能希望企业内搜索引擎索引），默认 noindex、可配置关闭。**更根本（属部署形态另记）**：管理后台本不该公网裸暴露，正路=内网/VPN/IP 白名单/独立域名不挂公网 DNS；noindex 三件套是「万一暴露也不被收录」的兜底。排期：低成本、可随便一片搭车或单列小片。
